...
class _MatchViewerScreenState extends State<MatchViewerScreen> {
  ...
  int currentMoveIndex = -1; // -1 significa posição inicial

  @override
  void initState() {
    super.initState();
    game = chess_logic.Chess(); // Começa um jogo novo em branco
  }
  ...
  void _irParaLance(int index) {
    ...
    // Aplica todos os lances do início até o momento atual que você clicou
    for (int i = 0; i <= index; i++) {
      String lance = widget.moves[i];
      
      // Quebra o texto "e2e4" ou "e7e8q"
      if (lance.length >= 4) {
        String de = lance.substring(0, 2);    // Pega o "e2"
        String para = lance.substring(2, 4);  // Pega o "e4"
        String? promocao = (lance.length == 5) ? lance.substring(4, 5) : null; // Pega o "q" se tiver
        
        // Manda o movimento no formato de dicionário que a biblioteca exige
        newGame.move({
          "from": de,
          "to": para,
  ...
  // REAPROVEITANDO SEU CÓDIGO DE IMAGENS
  ...
  Widget build(BuildContext context) {
    // Transforma o FEN atual em lista para desenhar
    List<String> board = [];
